...
-- Permet de purger proprement les seeds sans toucher aux vraies données utilisateur.
...
-- Fonction admin de purge — appelée par l'Edge Function wipe-seed-data.
-- Sécurité : SECURITY DEFINER + check superuser/service_role exigé côté caller.
...
DECLARE
  ...
  v_engagers BIGINT;
BEGIN
  -- Ordre important : on supprime du plus dépendant vers le moins dépendant
...
-- Vue de health-check pour alerter si des seeds traînent en prod.
