-- LE COOLDOWN NE PUNIT PLUS LES PANNES D'INFRASTRUCTURE.
--
-- Vécu par l'opératrice le 04/09 : ses signaux, victimes de la panne de
-- période Apify du matin, affichaient « réessai possible dans 24 h » APRÈS
-- la réparation. Le cooldown traitait un échec d'infrastructure — quota
-- fermé, réseau fournisseur en panne — comme un échec métier.
--
-- Or les deux classes ne se soignent pas pareil :
--   • un échec MÉTIER (résolution ambiguë, aucun persona trouvé) ne change
--     PAS en re-cliquant : le cooldown protège la facture d'une boucle de
--     réessais identiques. Il reste entier.
--   • un échec d'INFRASTRUCTURE ne dit RIEN du signal : la cause est
--     extérieure. Une fois l'infrastructure réparée, le réessai immédiat est
--     légitime — le faire attendre 24 h n'a jamais protégé personne.
--
-- `is_infrastructure_failure` classe par motifs CONNUS, et rend false dans
-- le doute : un motif inconnu est traité comme métier, donc freiné — se
-- tromper dans ce sens coûte une attente, se tromper dans l'autre coûterait
-- une boucle de dépense.

CREATE OR REPLACE FUNCTION public.is_infrastructure_failure(p_error text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
PARALLEL SAFE
...
AS $iff$
  SELECT coalesce(p_error, '') ILIKE ANY (ARRAY[
    '%Quota Apify refusé%',
    '%Période Apify non courante%',
    '%période Pappers non courante%',
...
COMMENT ON FUNCTION public.is_infrastructure_failure(text) IS
  'Vrai si le motif d echec est une panne d INFRASTRUCTURE (quota, reseau, '
  'service fournisseur) plutot qu un echec METIER (resolution, personas). '
  'Les echecs infra sont exemptes du cooldown de reessai ; dans le doute, '
  'false — un motif inconnu est freine, jamais rejoue en boucle.';
...
BEGIN
  ...
  IF p_job_type = 'contacts' THEN
    ...
    IF v_has_previous_job AND v_previous_job.status = 'failed' THEN
      -- Un échec d'INFRASTRUCTURE (quota fermé, réseau fournisseur) ne vaut
      -- aucun cooldown : la cause est extérieure au signal, et une fois
      -- l'infrastructure réparée, faire attendre 24 h n'apporte que de la
      -- frustration (constaté le 04/09 : « réessai possible dans 24 h » sur
      -- des signaux victimes de la panne de période). Les échecs MÉTIER
      -- gardent leur cooldown : eux ne changent pas en re-cliquant.
      IF p_cooldown_seconds > 0
         AND NOT public.is_infrastructure_failure(v_previous_job.error_message)
    ...
    IF NOT v_has_previous_job THEN
      ...
      ELSIF FOUND AND v_previous_enrichment.status = 'failed' THEN
        IF p_cooldown_seconds > 0
           AND NOT public.is_infrastructure_failure(v_previous_enrichment.error_message)
