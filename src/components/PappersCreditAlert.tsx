...
export function PappersCreditAlert({
  compact = false,
  showDetails = true,
}: PappersCreditAlertProps) {
      ...
      <CardContent className="p-4">
            ...
            <div>
              ...
              {showDetails && (
                <p className="text-sm text-muted-foreground">
                  {credits.isMeasured
                    ? `${credits.consumed.toLocaleString()} consommés + ${credits.reserved.toLocaleString()} réservés / ${credits.limit.toLocaleString()} ce mois`
                    : "Compteur autoritaire indisponible. Les scans restent bloqués."}
                </p>
              )}
            </div>
          </div>
        ...
        {credits.isBlocked && (
          <div className="mt-3 p-2 rounded bg-destructive/10 text-destructive text-sm flex items-center gap-2">
            ...
            {credits.isMeasured
              ...
              : "Compteur indisponible : les scans restent bloqués."}
          </div>
        )}
        {credits.isCritical && !credits.isBlocked && (
          <div className="mt-3 p-2 rounded bg-orange-500/10 text-orange-700 text-sm flex items-center gap-2">
            ...
            Attention : {100 - credits.percent}% des crédits restants. Planifiez
            vos scans.
          </div>
        )}
