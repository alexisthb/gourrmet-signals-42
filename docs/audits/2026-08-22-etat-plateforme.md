...
| **Ratio de détection Presse** | ~~Aucun corpus relu~~ **Amorcé le 22/08 au soir** : première annotation tracée (Sowell/expansion, non détectée — rappel labellisé 0/1 sur la cohorte `2026-08-22-fenetre-48h`, modèle et prompt consignés). Le diagnostic a basculé vers l'AMONT : sur 40 articles frais, ~97 % hors cible — l'aval (plancher CA + score ≥ 3, PR #41) filtre juste, c'est le sourcing qui gaspille. **Allowlist de 9 sources posée le 22/08 — retirée le 24/08, échec mesuré.** En 44 h sous filtre `domains=` : 15 articles, 0 signal, et surtout les DEUX meilleures sources (Journal des Entreprises, GlobeNewswire — 55 % des signaux historiques) ne revenaient PLUS DU TOUT, pendant que le bruit (dh.be, Zataz, Gameblog) continuait de passer. Le paramètre `domains=` de NewsAPI est défaillant des deux côtés pour ce cas : trop strict sur les sources utiles (indexation différente côté NewsAPI ?), poreux sur le reste. Retour arrière appliqué le 24/08 à 14:45 UTC selon le critère posé À L'AVANCE. La piste amont suivante, si on la reprend : POST-FILTRER par source AVANT l'analyse IA — le budget requêtes est identique de toute façon, on économiserait l'analyse sans jamais perdre une source |
